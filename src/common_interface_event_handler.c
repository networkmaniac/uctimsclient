...
void set_mode()
{
	...
	if((fp = fopen(imsua_addpath(".mode"),"rb")) != NULL) 
	{
		fgets(input, 10, fp);
		mode = (int)input[0] - 48;
	}
	...
	GtkWidget *mode_ims = lookup_widget(GTK_WIDGET(imsUA), "mode_ims");
	GtkWidget *mode_sip = lookup_widget(GTK_WIDGET(imsUA), "mode_sip");

	if (mode == IMS_MODE)
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(mode_ims), 1);
	else if (mode == SIP_MODE)
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(mode_sip), 1);
}
...
void reject_call()
{
	...
	//if ((ca = find_call(current_dialog_id)) != NULL)
