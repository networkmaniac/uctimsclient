...
void preferences_set_preferences_dialog()
{
	if(preferences_open == 0)
	{
		...
		gtk_entry_set_text(GTK_ENTRY(profile_name_widget), pref->name);
...
void preferences_get_preferences_dialog(gpointer button)
{
	...
	strcpy(pref->name, gtk_entry_get_text(GTK_ENTRY(profile_name_widget)));
	...
	strcpy(pref->impu, gtk_entry_get_text(GTK_ENTRY(ims_public_ui_widget)));
	strcpy(pref->impi, gtk_entry_get_text(GTK_ENTRY(ims_private_ui_widget)));
	strcpy(pref->pcscf, gtk_entry_get_text(GTK_ENTRY(ims_pcscf_widget)));
	strcpy(pref->realm, gtk_entry_get_text(GTK_ENTRY(ims_realm_widget)));
	strcpy(pref->password, gtk_entry_get_text(GTK_ENTRY(ims_password_widget)));
	...
	pref->audio_codec_primary = gtk_combo_box_get_active(GTK_COMBO_BOX(media_1st_codec_widget));
	pref->audio_codec_secondary = gtk_combo_box_get_active(GTK_COMBO_BOX(media_2nd_codec_widget));
	pref->dtmf = gtk_combo_box_get_active(GTK_COMBO_BOX(media_dtmf_events_widget));
	pref->media_interface = gtk_combo_box_get_active(GTK_COMBO_BOX(media_interface_widget));

	/* Get the IP address if we are not using the default */
	if(strcmp("Default", gtk_combo_box_get_active_text(GTK_COMBO_BOX(media_interface_widget))))
	{
		strcpy(pref->local_audio_ip, gtk_combo_box_get_active_text(GTK_COMBO_BOX(media_interface_widget)));
		strcpy(pref->local_video_ip, gtk_combo_box_get_active_text(GTK_COMBO_BOX(media_interface_widget)));
	}

	pref->video_bw = gtk_combo_box_get_active(GTK_COMBO_BOX(media_video_bw_widget));
...
int preferences_get_preferences_from_xml_file(char *filename)
{
	...
	{
		g_warning("Error opening preferences file: %s", imsua_addpath(filename));
		xmlFreeDoc( doc );
		return FALSE;
	}
...
int preferences_write_preferences_to_xml_file(char *filename)
{
	xmlDocPtr doc;
	...
	xmlSaveFormatFile (imsua_addpath(filename), doc, 1);
	

}


int preferences_set_default_preferences()
{
	...
	pref->local_msrp_port = 5000 + rand()/(int)(((unsigned)RAND_MAX + 1) / 30000);
}
