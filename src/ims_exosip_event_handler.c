...
void ims_process_registration_200ok(eXosip_event_t *je)
{
	...
	if (!is_message_deregister)
	{
		...
		eXosip_lock();
		i = eXosip_subscribe_build_initial_request(&subscribe, pref->impu, pref->impu, imsua_add_lr_to_route(add_sip_scheme(pref->pcscf)), "reg", 600000);
		eXosip_unlock();
...
void ims_process_incoming_invite(eXosip_event_t *je)
{
	...
	if (!ims_message_requires_preconditions(je->request))
	{
		...
		sdp_complete_ims(je->request, &ringing, ca);
	...
	else
	{
		...
		sdp_complete_ims(je->request, &session_progress, ca);
		...
		imsua_set_message_display("183 Session Progress", 1);
	}
}
...
void ims_process_18x(eXosip_event_t *je)
{
	...
	// send a PRACK if the provisional response requires preconditions
	if (ims_message_requires_100rel(je->response))
	{
		...
		sdp_complete_ims(je->response, &prack, ca);
...
void ims_process_prack(eXosip_event_t *je)
{
	...
	if (!ca->media_negotiated)
		sdp_complete_ims(je->request, &answer_200ok, ca);
...
void ims_process_2xx(eXosip_event_t *je)
{
	...
	if (ca->qos_confirm && !ca->sent_update)
	{
		...
		sdp_complete_ims(ca->prov_resp, &update, ca);
...
void ims_process_200ok(eXosip_event_t *je)
{
	...
	if (!ca->media_negotiated)
		sdp_complete_ims(je->response, &ack, ca);
...
void ims_process_302 (eXosip_event_t *je)
{
	...
	else if (eXosip_call_build_initial_invite (&invite,uri_entry,from_string,imsua_add_lr_to_route(add_sip_scheme(pref->pcscf)),"IMS Call"))
	{
	...
	sdp_complete_ims(NULL, &invite, NULL);
