...
create_videoWin (void)
{
  ...
  gtk_window_set_title (GTK_WINDOW (videoWin), _("IPTV Display"));
...
static gboolean expose_cb(GtkWidget * widget, GdkEventExpose * event, gpointer data)
{
	...
	// gst_video_overlay_set_window_handle(GST_VIDEO_OVERLAY(data), GDK_WINDOW_XWINDOW(widget->window));
...
{
	char *ringing_tone = "rings/ctu24ringtone.mp3";
...
int initialiseBackgroundVideoPipeline()
{
	...
	gst_video_overlay_set_window_handle(GST_VIDEO_OVERLAY(screen_sink), GDK_WINDOW_XWINDOW(local_cam_widget->window));
...
int initialiseVideoTxPipeline(Call *ca)
{
	...
	GstElement *pipeline, *screen_sink,*camera_src;
	...
	/* Check that elements are correctly initialized */
	if(!(pipeline && camera_src && csp_filter && tee && screen_sink && h263_encoder && rtp_h263_payloader && udp_sink && screen_queue && udp_queue))
	{
	 ...
	 * linking them */
	gst_bin_add_many(GST_BIN(pipeline), camera_src, tee, csp_filter2, csp_filter, udp_queue, h263_encoder, rtp_h263_payloader, udp_sink, screen_queue, screen_sink, NULL);
	...
	if(!gst_element_link_many(tee, csp_filter, screen_queue, screen_sink, NULL))
	{
		g_warning("Problem linking elements for the video transmitting preview section\n");
		return FALSE;
	}

	if(!gst_element_link_many(tee, csp_filter2, udp_queue, h263_encoder, rtp_h263_payloader, udp_sink, NULL))
	{
		g_warning("Problem linking elements for the video transmitting sending section\n");
		return FALSE;
	}
...
int initialiseVideoRxPipeline(Call *ca)
{
	...
	csp_filter = gst_element_factory_make("ffmpegcolorspace", "csp_filter");
	// screen_sink = gst_element_factory_make("sink", "screen_sink");
	screen_sink = gst_element_factory_make("ximagesink", "screen_sink");
	...
	{
		g_critical("Can't link elements for the video receiving pipeline.\n");
		return FALSE;
	}
	gst_caps_unref(caps);
	...
	GtkWidget *remote_cam_widget;
	remote_cam_widget = lookup_widget(GTK_WIDGET(imsUA), "remote_cam");

	/* As soon as screen is exposed, window ID will be advised to the sink */
	// g_signal_connect(remote_cam_widget, "expose-event", G_CALLBACK(expose_cb), screen_sink);
	gst_video_overlay_set_window_handle(GST_VIDEO_OVERLAY(screen_sink), GDK_WINDOW_XWINDOW(remote_cam_widget->window));
...
int initialiseIptvVideoPipeline(Call *ca)
{
	...
	gst_video_overlay_set_window_handle(GST_VIDEO_OVERLAY(screen_sink), GDK_WINDOW_XWINDOW(videoWin->window));
