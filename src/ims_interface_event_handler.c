...
gint ims_send_register()
{
	...
	if ((reg_id < 0) || (preferences_changed == 1))
	{
		...
		osip_message_set_route(reg, imsua_add_lr_to_route(add_sip_scheme(pref->pcscf)));
		...
		g_timeout_add ((1000 * (REG_EXPIRE - 32)), ims_send_register, NULL);
...
void ims_call_answer()
{
	...
	if ((ca->most_recent_message) && (!ca->media_negotiated))
		sdp_complete_ims(ca->most_recent_message, &answer, ca);
...
void ims_call_reinvite()
{
	...
	*/
	...
	/* Put in our media preferences */
	sdp_complete_ims(NULL, &reinvite, NULL);
